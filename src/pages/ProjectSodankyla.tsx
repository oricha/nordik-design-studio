...
const images = [project2, project1, project3];
...
const ProjectSodankyla = () => {
  const [activeImage, setActiveImage] = useState(0);
  const [customizations, setCustomizations] = useState<CustomOption[]>([
    { name: "Muebles y electrodomésticos de cocina", items: 6, price: 6000, added: false },
    { name: "Electrodomésticos de baño", items: 5, price: 5500, added: false },
    { name: "Calefacción", items: 2, price: 6500, added: false },
  ]);
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <section className="relative w-full h-[70vh] min-h-[500px] overflow-hidden">
          <img src={images[activeImage]} alt="Vivienda modelo Sodankylä" className="w-full h-full object-cover" />
          ...
          <div className="absolute bottom-0 left-0 p-8 md:p-16 max-w-lg">
            ...
            <h1 className="text-4xl md:text-5xl font-bold text-background mb-4">Sodankylä</h1>
            <div className="flex gap-2 mb-4">
              <span className="bg-accent text-accent-foreground text-xs font-semibold px-3 py-1 rounded-full">HOUSE</span>
            ...
            <div className="flex gap-6 text-background/80 text-sm mb-6">
              <span className="flex items-center gap-1"><Maximize2 className="w-4 h-4" /> 180 m²</span>
              <span className="flex items-center gap-1"><BedDouble className="w-4 h-4" /> 5 habitaciones</span>
              <span className="flex items-center gap-1"><Bath className="w-4 h-4" /> 3 baños</span>
            </div>
            <p className="text-background/70 text-sm">
              Tiempo de construcción: <strong className="text-background">6 meses</strong>
            </p>
          ...
          <div className="absolute bottom-8 right-8 hidden md:flex gap-2">
            {images.map((img, i) => (
              <button key={i} onClick={() => setActiveImage(i)} className={`w-16 h-12 rounded-md overflow-hidden border-2 transition-all ${activeImage === i ? "border-accent" : "border-background/30"}`}>
        ...
        <section className="section-padding bg-background">
          <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-12 items-start">
            <div>
              <img src={project1} alt="Sodankylä — vista lateral" className="w-full rounded-xl" loading="lazy" />
            </div>
            <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[gallery1, gallery2, gallery3, gallery4].map((img, i) => (
                <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="aspect-[4/3] rounded-xl overflow-hidden">
                  <img src={img} alt={`Sodankylä · imagen ${i + 1}`} className="w-full h-full object-cover hover:scale-105 transition-transform duration-500" loading="lazy" />
                </motion.div>
              ))}
            </div>
          </div>
        ...
        <section className="section-padding bg-background">
          <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-12 items-start">
            ...
            <div className="sticky top-24">
              <img src={project2} alt="Sodankylä" className="w-full rounded-xl mb-6" loading="lazy" />
...
export default ProjectSodankyla;
