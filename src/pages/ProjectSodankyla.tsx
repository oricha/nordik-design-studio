...
const ProjectSodankyla = () => {
  ...
  const [customizations, setCustomizations] = useState<CustomOption[]>([
    { name: "Kitchen furniture & Appliances", items: 6, price: 6000, added: false },
    { name: "Bathroom appliances", items: 5, price: 5500, added: false },
    { name: "Heating", items: 2, price: 6500, added: false },
  ]);
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <section className="relative w-full h-[70vh] min-h-[500px] overflow-hidden">
          <img src={images[activeImage]} alt="Sodankylä house" className="w-full h-full object-cover" />
          ...
          <div className="absolute bottom-0 left-0 p-8 md:p-16 max-w-lg">
            ...
            <div className="flex gap-6 text-background/80 text-sm mb-6">
              ...
              <span className="flex items-center gap-1"><BedDouble className="w-4 h-4" /> 5 bedrooms</span>
              <span className="flex items-center gap-1"><Bath className="w-4 h-4" /> 3 bathrooms</span>
            </div>
            <p className="text-background/70 text-sm">
              Construction time: <strong className="text-background">6 months</strong>
            </p>
        ...
        <section className="section-padding bg-background">
          <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-12 items-start">
            <div>
              <img src={project1} alt="Sodankylä side view" className="w-full rounded-xl" loading="lazy" />
            </div>
            <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[gallery1, gallery2, gallery3, gallery4].map((img, i) => (
                <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="aspect-[4/3] rounded-xl overflow-hidden">
                  <img src={img} alt={`Sodankylä photo ${i + 1}`} className="w-full h-full object-cover hover:scale-105 transition-transform duration-500" loading="lazy" />
                </motion.div>
              ))}
            </div>
          </div>
